...
function collect() {
  ...
  for (const file of files) {
    ...
    for (const line of text.split(/\r?\n/)) {
      ...
      if (!g) { g = { key, samples: [] }; groups.set(key, g); }
      g.samples.push(ms);
      used++;
    }
  }
  ...
  return { files: files.map((f) => path.basename(f)), scanned, used, rows };
}
...
function printTable(res) {
  ...
  console.log(pad('事件', w) + lpad('次数', 6) + lpad('均值', 7) + lpad('中位', 7) + lpad('p95', 7) + lpad('最小', 7) + lpad('最大', 7));
  console.log('-'.repeat(w + 41));
  for (const r of res.rows) {
    console.log(pad(r.key, w) + lpad(r.n, 6) + lpad(r.mean, 7) + lpad(r.median, 7) + lpad(r.p95, 7) + lpad(r.min, 7) + lpad(r.max, 7));
  }
  const all = res.rows.reduce((a, r) => ({ n: a.n + r.n, sum: a.sum + r.mean * r.n }), { n: 0, sum: 0 });
  console.log('-'.repeat(w + 41));
  console.log(pad('全部事件', w) + lpad(all.n, 6) + lpad(Math.round(all.sum / all.n), 7) + '   ← 总体均值');
  console.log('\n注:样本含测试套件驱动的高负载会话,与日常浏览不可直接比较;要日常基线请清空 logs 后正常使用一段时间再统计。\n');
}
...
function writeReport(res) {
  ...
  lines.push('');
  lines.push('| 事件 | 次数 | 均值 | 中位 | p95 | 最小 | 最大 |');
  lines.push('|---|---:|---:|---:|---:|---:|---:|');
  for (const r of res.rows) {
    lines.push(`| \`${r.key}\` | ${r.n} | **${r.mean} ms** | ${r.median} | ${r.p95} | ${r.min} | ${r.max} |`);
  }
  const all = res.rows.reduce((a, r) => ({ n: a.n + r.n, sum: a.sum + r.mean * r.n }), { n: 0, sum: 0 });
  if (all.n) {
    lines.push('');
    lines.push(`**全部事件总体均值:${Math.round(all.sum / all.n)} ms(${all.n} 次采样)**`);
  }
  lines.push('');
  lines.push('## 解读');
  lines.push('');
